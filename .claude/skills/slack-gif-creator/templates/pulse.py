    ...
    for i in range(num_frames):
        ...
        if pulse_type == "smooth":
            ...
            if phase < 0.15:
                ...
                # Second pump (smaller)
                scale = interpolate(
                    min_scale, (min_scale + max_scale) / 2, (phase - 0.25) / 0.10, "ease_out"
                )
            elif phase < 0.45:
                # Second release
                scale = interpolate(
                    (min_scale + max_scale) / 2, min_scale, (phase - 0.35) / 0.10, "ease_in"
                )
            else:
                # Rest period
                scale = min_scale
        ...
        if object_type == "emoji":
            ...
            draw_circle(
                frame, center=center_pos, radius=current_radius, fill_color=object_data["color"]
            )
